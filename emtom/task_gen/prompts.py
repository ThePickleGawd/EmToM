...
{test_gate_line}
- `taskgen verify_task` is the final submission gate: it runs `gpt-5.4`, `claude-sonnet-4-6`, and `gemini-flash` in standard mode. You may submit if at least 1 of the 3 models fails.
...
{pddl_checklist}{test_checklist}- After `taskgen test_task` passes, run `taskgen verify_task`. Submit only if at least 1 of `gpt-5.4`, `claude-sonnet-4-6`, and `gemini-flash` fails.
## References
...
def build_external_taskgen_prompt(
    *,
    ...
    workflow_lines.append(
        f"{final_step_number + 2}. Submit only if verification passes, meaning at least 1 of `gpt-5.4`, `claude-sonnet-4-6`, and `gemini-flash` fails in standard mode."
    )
