...
class EmtomPlanner(Planner):
    """
    EMTOM planner with custom ReAct-style logging.

    Based on LLMPlanner but with exploration-style output formatting.
    """
    ...
    def get_next_action(
        ...
        if self.replan_required:
            ...
            # ReAct-style logging (like exploration)
            AGENT_COLORS = ["\033[94m", "\033[92m", "\033[93m", "\033[95m", "\033[96m", "\033[91m"]
            RESET = "\033[0m"
            for uid, action_tuple in high_level_actions.items():
                if action_tuple and action_tuple[0]:
                    action_name, action_arg, _ = action_tuple
                    color = AGENT_COLORS[uid % len(AGENT_COLORS)]
                    obs = responses.get(uid, "")
                    ...
                    # Print Action
                    print(f"{color}Agent_{uid}_Action:{RESET} {action_name}[{action_arg}]", flush=True)
                    print("Assigned!", flush=True)

                    # Print Observation
                    if obs:
                        print(f"{color}Agent_{uid}_Observation:{RESET} {obs}", flush=True)
        else:
