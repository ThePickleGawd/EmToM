...
print_usage() {
    ...
    echo "  benchmark-suite Benchmark one task set across multiple models"
    ...
    echo "  --model MODEL        LLM model name (default: gpt-5.4, provider auto-detected)"
    ...
    echo -e "  │ GPT-5.4 (default)         │ ${GREEN}gpt-5.4${NC}            │"
    echo -e "  ├───────────────────────────┼────────────────────┤"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        benchmark-suite)
            ...
            shift
            python -m emtom.scripts.benchmark_suite "$@"
            exit $?
            ;;
