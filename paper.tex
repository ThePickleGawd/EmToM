...
Table~\ref{tab:tom_orders} illustrates the successive orders of Theory of Mind with everyday analogies and corresponding \shortname{} task examples. Each order adds a layer of reasoning about what another agent knows.
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.4cm} >{\centering\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
\toprule
\textbf{Order} & \textbf{Formal} & \textbf{Everyday analogy} & \textbf{EmToM example} \\
\midrule

\textbf{0} \newline \textit{No ToM}
& $\phi$
& You walk to the kitchen and open the fridge because you are thirsty. No one else is involved.
& An agent navigates to a cabinet and opens it. No coordination needed. \\

\addlinespace[3pt]

\textbf{1} \newline \textit{Self-aware}
& $\mathcal{K}_a(\phi)$
& You cannot reach the top shelf, so you ask your roommate to check if the jar is there.
& Agent~0 is barred from the kitchen. It asks Agent~1 to check whether the fridge is open and report back. Agent~0 models \emph{its own} knowledge gap, not Agent~1's mind. \\

\addlinespace[3pt]

\textbf{2} \newline \textit{Other-aware}
& $\mathcal{K}_a\!\bigl(\mathcal{K}_b(\phi)\bigr)$
& Your roommate left before the package arrived. You know \emph{they} don't know it is here, so you text them.
& Agent~0 must ensure Agent~2 knows the bowl is on the table. Agent~2 is in another room and has not seen it. Agent~0 reasons about what Agent~2 lacks and relays the information. \\

\addlinespace[3pt]

\textbf{3} \newline \textit{Recursive}
& $\mathcal{K}_a\!\bigl(\mathcal{K}_b\!\bigl(\mathcal{K}_c(\phi)\bigr)\bigr)$
& You need to confirm your manager knows the client received the contract. Your assistant sent it, so you ask the assistant to confirm they told the manager.
& Agent~0 must know that Agent~1 knows that Agent~2 knows the wardrobe is open. This requires a planned relay chain across three agents with restricted communication. \\

\addlinespace[3pt]

\textbf{4+} \newline \textit{Self-ref.}
& $\mathcal{K}_a\!\bigl(\mathcal{K}_b\!\bigl(\cdots\bigr)\bigr)$
& Poker: you think your opponent thinks you are bluffing, so you bet high---but they may know that you know they think this.
& A competitive task requires $\mathcal{K}_{a_1}\!\bigl(\mathcal{K}_{a_0}\!\bigl(\mathcal{K}_{a_1}\!\bigl(\mathcal{K}_{a_0}(\phi)\bigr)\bigr)\bigr)$: a self-referential loop through the same two agents. \\

\bottomrule
\end{tabularx}
\caption{Orders of Theory of Mind. Each row adds a layer of reasoning about other agents' epistemic states. Frontier models reliably handle order~1 but fail at order~2 and above (Section~\ref{sec:behaviors}).}
